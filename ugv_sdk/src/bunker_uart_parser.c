...
typedef enum
{
    ...
    WAIT_FOR_CHECKSUM
} BunkerSerialDecodeState;

#define PAYLOAD_BUFFER_SIZE (BUNKER_FRAME_SIZE * 2)
...
// internal functions
static bool ParseChar(uint8_t c, BunkerMessage *msg);
static uint8_t CalcBufferedFrameChecksum();
static bool ConstructStatusMessage(BunkerMessage *msg);
static bool ConstructControlMessage(BunkerMessage *msg);
...
//            buf[4] = UART_FRAME_MOTOR4_DRIVER_STATUS_ID;
    ...
    buf[12] = CalcBunkerUARTChecksum(buf, buf[2] + FRAME_SOF_LEN);
...
{
    static BunkerMessage decoded_msg;
...
bool ParseChar(uint8_t c, BunkerMessage *msg)
{
    static BunkerSerialDecodeState decode_state = WAIT_FOR_SOF1;
...
bool ConstructControlMessage(BunkerMessage *msg)
{
    if (msg == NULL)
        return false;
...
bool ConstructStatusMessage(BunkerMessage *msg)
{
    if (msg == NULL)
        return false;
