/* 
 * hunter_uart_parser.c
 * 
 * Created on: Apr 01, 2020 09:49
 * Description: 
 * 
...
// internal functions
static bool ParseChar(uint8_t c, HunterMessage *msg);
static uint8_t CalcBufferedFrameChecksum();
static bool ConstructStatusMessage(HunterMessage *msg);
static bool ConstructControlMessage(HunterMessage *msg);
...
void EncodeScoutMsgToUART(const HunterMessage *msg, uint8_t *buf, uint8_t *len)
{
    // SOF
    buf[0] = FRAME_SOF1;
    buf[1] = FRAME_SOF2;
...
bool DecodeScoutMsgFromUART(uint8_t c, HunterMessage *msg)
{
    static HunterMessage decoded_msg;
...
bool ParseChar(uint8_t c, HunterMessage *msg)
{
...
bool ConstructControlMessage(HunterMessage *msg)
{
    if (msg == NULL)
        return false;
...
bool ConstructStatusMessage(HunterMessage *msg)
{
    if (msg == NULL)
        return false;
